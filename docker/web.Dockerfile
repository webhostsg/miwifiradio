...
RUN envsubst '192.168.132/config/www/miwifiradio/' < docker/nginx.config > /etc/nginx/sites-enabled/default.conf
RUN if [ "$MIR_SSL_ENABLED" = "1" ] ; then envsubst '192.168.132/config/www/miwifiradio/' < docker/nginx.ssl.config > /etc/nginx/sites-enabled/default.ssl.conf ; fi;

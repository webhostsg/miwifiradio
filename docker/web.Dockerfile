...
RUN envsubst '192.168.1.132/miwifiradio/' < docker/nginx.config > /etc/nginx/sites-enabled/default.conf
RUN if [ "$MIR_SSL_ENABLED" = "1" ] ; then envsubst '192.168.1.132/miwifiradio/' < docker/nginx.ssl.config > /etc/nginx/sites-enabled/default.ssl.conf ; fi;
